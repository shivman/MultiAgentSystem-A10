...
def caption_image(img_url_or_path: str) -> str:
    ...
    if img_url_or_path.startswith("http"):
        # This is a URL, don't try to create local path
        pass
    else:
    ...
    try:
        ...
        if "403" in str(e) or "Forbidden" in str(e):
            return f"[Image access forbidden: {img_url_or_path}]"
        elif "404" in str(e) or "Not Found" in str(e):
            return f"[Image not found: {img_url_or_path}]"
        elif "timeout" in str(e).lower():
            return f"[Image processing timeout: {img_url_or_path}]"
        else:
...
def replace_images_with_captions(markdown: str) -> str:
    def replace(match):
        ...
        try:
            # Skip processing if src is empty or invalid
            if not src or src.strip() == "":
                return f"[Invalid image source]"
            
            # Skip processing if src is a directory
            if src.endswith('/') or src.endswith('\\'):
                return f"[Directory path, not an image: {src}]"
            
            caption = caption_image(src)
            # Attempt to delete only if local and file exists
            if not src.startswith("http"):
                img_path = Path(__file__).parent / "documents" / src
                if img_path.exists() and img_path.is_file():  # Check it's actually a file
                    try:
                        img_path.unlink()
            ...
            mcp_log("WARN", f"Image processing failed: {e}")
